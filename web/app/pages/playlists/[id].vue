...
const tracks = useObservable(
  from(
    liveQuery(
      async () => {
        const pairs = await dexieStorage.playlistTracks.where("playlistId").equals(playlistId.value).toArray()
        const items = await Promise.all(pairs.map(p => dexieStorage.tracks.get(p.trackId)))
