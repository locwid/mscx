...
<template>
  <UModal
    ...
    :ui="{
      content:
        'h-screen w-full max-w-lg mx-auto bg-black shadow-lg ring ring-default flex flex-col focus:outline-none overflow-hidden rounded-none',
    }"
  >
    <template #content>
      <div class="relative flex flex-col h-full">
        ...
        <div class="absolute inset-0 flex flex-col p-6 z-10">
          ...
          <div class="flex-1" />
          <div
            class="flex gap-8 flex-col backdrop-blur-sm bg-black/80 rounded-xl px-4 py-6"
          >
            <div class="w-full">
              <Transition name="fade" mode="out-in">
                <div class="text-center" :key="track.id">
                  <h2
                    class="text-2xl font-bold truncate text-white drop-shadow-lg"
                  >
                    {{ track.name }}
                  </h2>
                </div>
