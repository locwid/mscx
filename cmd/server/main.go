...
func main() {
	...
	tagService := service.MakeTagService(db)
	tagController := controller.MakeTagController(tagService)
	tag := api.Group("/tag")
	tag.GET("", tagController.GetList)
	tag.POST("", tagController.Create)
	tag.DELETE("/:id", tagController.Delete)
	tag.POST("/:id/track/:trackId", tagController.AddTrack)
	tag.DELETE("/:id/track/:trackId", tagController.DeleteTrack)
